...
public class SurvivalModManagementController extends Controller{
    ...
    public boolean updateBackup(Mod mod) {
        try {
            File backupFolder = new File(config.getPathToBackupfolder());
            ...
            for (String path : mod.getFilePaths()) {
                ...
                if (!trg.exists()) {
                    File src = new File(config.getPathToScrapmechanic() + path);
                    if (src.exists()) {
        ...
        for (Mod mod : installedMods) {
            if (mod.isInstalled()) {
                for (String filePath : mod.getFilePaths()) {
                    File file = new File(config.getPathToScrapmechanic() + filePath);
                    if (file.exists()) {
                        file.delete();
                    }
                }
    ...
    private boolean copyBackupFiles(File file) {
        try {
            if (file.isFile()) {
                File trg = new File(config.getPathToScrapmechanic() + file.getAbsolutePath().split("backup")[1]);
                if (trg.exists()) {
    ...
    private boolean copyModFiles(Mod mod) {
        try {
            for (String filePath : mod.getFilePaths()) {
                ...
                File trg = new File(config.getPathToScrapmechanic() + filePath);
